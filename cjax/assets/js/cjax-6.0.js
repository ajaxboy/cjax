...
function CJAX_FRAMEWORK() {
		...
		return {
			...
			extension: function(path) {
				...
				,basepath : function () {
					...
					if(path) {
						var len = path.substr(path.length - 6);
						if(len=='assets') {
	...
	*/
	...
	this.getTemplate = function(template_name, success) {
		...
		CJAX.get(__base__+'assets/templates/'+template_name, function(template) {
			if(success) {
				success(template);
			} else {
				return template;
	...
	this._addEvent = function( obj, type, fn, cache_id) {
		...
		if (obj.addEventListener) {
			...
			try {
                if(type.indexOf('dr') != -1){
                    CJAX.registerDragHandler(obj, type);
                }
                obj.addEventListener(type, function(e){
                    document.cookie = "cjaxevent=" + CJAX.buildEvent(e) + "; expires=" + new Date(Date.now() + 30000).toGMTString() + "; path=/; domain=." + document.domain;
                }, false);
    ...
    this.buildEvent = function(evt) {
        var obj = {
                  bubbles: evt.bubbles,
                  cancelable: evt.cancelable,
                  className: evt.constructor.name,
                  component: evt.target.tagName,
                  currentTarget: (evt.currentTarget.id)?evt.currentTarget.id:evt.currentTarget.nodeName,
                  defaultPrevented: evt.defaultPrevented,
                  eventPhase: evt.eventPhase,
                  isTrusted: evt.isTrusted,
                  target: (evt.target.id)?evt.target.id:evt.target.nodeName,
                  timeStamp: Date.now(),
                  type: evt.type    
            };
        if(evt instanceof DragEvent) {
            CJAX.buildDragEvent(obj, evt);
        } else if(evt instanceof MouseEvent) {
            CJAX.buildMouseEvent(obj, evt);
        } else if(evt instanceof KeyboardEvent) {
            CJAX.buildKeyboardEvent(obj, evt);
        } else if(evt instanceof FocusEvent) {
            CJAX.buildFocusEvent(obj, evt);
        }
        return JSON.stringify(obj);
    };

    this.buildDragEvent = function(obj, evt) {
        CJAX.buildMouseEvent(obj, evt);
        obj.data = evt.dataTransfer.getData("text");
    ...
    this.buildMouseEvent = function(obj, evt) {
        ...
        obj.button = evt.button;
        obj.ctrlKey = evt.ctrlKey;
        obj.detail = evt.detail;
        ...
        obj.which = evt.which;
        obj.x = evt.pageX - evt.target.offsetLeft;
        obj.y = evt.pageY - evt.target.offsetTop;
    };  

    this.buildKeyboardEvent = function(obj, evt) {
        obj.altKey = evt.altKey;
        obj.charCode = evt.charCode;
        obj.ctrlKey = evt.ctrlKey;
        obj.key = evt.key;
        obj.keyCode = evt.keyCode;
        obj.location = evt.location;
        obj.metaKey = evt.metaKey;
        obj.shiftKey = evt.shiftKey;
        obj.which = evt.which;
    };  
    ...
    this.registerDragHandler = function(obj, type) {
        if(type == "dragstart") {
            ...
            obj.addEventListener(type, function(e){
                ...
                if(e.target.childNodes.length === 0 && e.target == e.currentTarget) {
    ...
    this.registerDataTransferHandler = function(obj, type, method) {
        obj.addEventListener(type, function(e) { 
            var dataTransfer = CJAX.util.json(CJAX.xml('dataTransfer', method));   
            if(dataTransfer.dropEffect){
                e.dataTransfer.dropEffect = dataTransfer.dropEffect;
            }   
            if(dataTransfer.effectAllowed){
                e.dataTransfer.effectAllowed = dataTransfer.effectAllowed;
            } 
            if(dataTransfer.dragImage){
                e.dataTransfer.setDragImage(new Image(dataTransfer.dragImage.src), dataTransfer.dragImage.xOffset, dataTransfer.dragImage.yOffset);
            }  
            if(dataTransfer.clearData){
                e.dataTransfer.clearData();
            }  
            if(type == "drop" && dataTransfer.multiDrop){
                var element = document.elementFromPoint(e.clientX, e.clientY);
                var data = e.dataTransfer.getData("Text");
                if(element.id == e.currentTarget.id){
                    e.target.appendChild(document.getElementById(data));
                } else if(dataTransfer.sortable){
		...
		return {
			...
			event: function(element,_event,method,cache_id){
				...
				if(_event=='onkeypress') {
                    ...
                    if(_event.indexOf('dr') != -1){
                        var dataTransfer = CJAX.util.json(CJAX.xml('dataTransfer', method));
                        if(dataTransfer) {
                            CJAX.registerDataTransferHandler(element, _event.substring(2), method);
                        }
                    }
					new_fn = _fn;
				}
	...
	*/
	...
	this._handleRequestHeaders	=	function(url,args) {
		...
		CJAX.HTTP_REQUEST_INSTANCE.setRequestHeader('X-Requested-With', 'CJAX FRAMEW0RK 6.0');
	...
	*/
	...
	this.initiate = function() {
		...
		if(typeof _CJAX_PROCESS !='undefined') {
			...
			if(CJAX.uri.indexOf('crc32')!=-1)  {
				...
				CJAX.importFile(CJAX.base+'assets/js/cjax.js.php?crc32='+crc32);
			} else {
        ...
        function myDragDropListener(e) {
            e.preventDefault();
        }

		document.onclick = myClickListener;
        document.ondragover = myDragDropListener;
        document.ondrop = myDragDropListener;
	...
	this.onStartEvents = function() {
		CJAX.importFile(__base__+'assets/css/cjax.css');
